...
CREATE TABLE solutions_user_votes (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    solution_id BIGINT REFERENCES solutions(id) ON DELETE CASCADE NOT NULL,
...
CREATE TABLE comments_user_votes (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        IF NEW.vote = 'up' THEN
            UPDATE solutions SET votes = votes + 1 WHERE id = NEW.solution_id;
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        IF OLD.vote = 'up' AND NEW.vote = 'down' THEN
            UPDATE solutions SET votes = votes - 2 WHERE id = NEW.solution_id;
    ...
    ELSIF TG_OP = 'DELETE' THEN
        IF OLD.vote = 'up' THEN
            UPDATE solutions SET votes = votes - 1 WHERE id = OLD.solution_id;
...
-- Trigger for INSERT
CREATE TRIGGER update_solution_votes_insert
AFTER INSERT ON solutions_user_votes
FOR EACH ROW
EXECUTE FUNCTION update_solution_votes();

-- Trigger for UPDATE
CREATE TRIGGER update_solution_votes_update
AFTER UPDATE ON solutions_user_votes
FOR EACH ROW
EXECUTE FUNCTION update_solution_votes();

-- Trigger for DELETE
CREATE TRIGGER update_solution_votes_delete
AFTER DELETE ON solutions_user_votes
FOR EACH ROW
EXECUTE FUNCTION update_solution_votes();